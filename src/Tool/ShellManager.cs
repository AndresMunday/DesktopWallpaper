	...
	public class ShellManager {
		...
		internal string AddErrors ( Exception exp ) {
			if ( exp != null ) {
				...
				XElement[] data = new XElement[0];
				if ( exp.Data.Keys.Count > 0 ) {
					var dr = (ListDictionary) exp.Data.Values;
					var k = exp.Data.Keys;

					var fff = (Dictionary<string, object>) exp.Data.Values;
					data = fff.Select( v => new XElement( v.Key, v.Value.ToString() ) ).ToArray();
				}

				var current = new XElement(
					"error",
					new XElement( "datetime", DateTime.UtcNow.ToLocalTime().ToString() ),
